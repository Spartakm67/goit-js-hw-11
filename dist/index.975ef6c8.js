...
async function getGallery(inputValue) {
    try {
        maxLoadQ -= 40;
        page += 1;
        const response = await (0, _axiosDefault.default).get(`${BASE_URL}?key=${KEY_PIXABAY}&q=${inputValue}&image_type=photo&orientation=horizontal&safesearch=true&per_page=${per_page}&page=${page}`);
        let hits = response.data.hits;
        const totalHits = response.data.totalHits;
        console.log(response);
        if (hits.length) markupGallery(response.data.hits);
        else {
        ...
        if (page === 1 && hits.length) (0, _notiflixDefault.default).Notify.info(`Hooray! We found ${totalHits} images.`);
        if (page > 1) {
...
function markupGallery(userSearchArray) {
    const markup = userSearchArray.map(({ webformatURL , largeImageURL , tags , likes , views , comments , downloads ,  })=>`<a class="gallery-box" href="${largeImageURL}"><div class="photo-card">
...
},{"./css/styles.css":"1CY4s","notiflix":"5WWYd","axios":"jo6P5","simplelightbox":"9ydBq","simplelightbox/dist/simple-lightbox.min.css":"kaxSc","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"5WWYd":[function(require,module,exports) {
var global = arguments[3];
